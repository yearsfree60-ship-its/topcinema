...
# [إضافة — تشخيص بروتوكولي اختياري] راجع تحليل تسريب CDP Runtime.enable
# المرفق بجولة التشخيص هذه (مصادر 2026: playwright-extra-plugin-stealth
# معطَّل فعليًا ضد Cloudflare/DataDome الحديثة لأن السبب بروتوكولي لا JS).
# مسابر إضافية (تسريب Runtime.enable + توقيع sourceURL) — أبطأ من التشخيص
# العادي (فحص إضافي بكل جلسة متصفح، بمرَّتين: بstealth وبلاه)، لذا تعمل
# فقط بتفعيل صريح لهذا الخيار — خيار منفصل، لا تلقائي بكل تشخيص عادي.
DEEP_DIAGNOSTIC = os.environ.get("DEEP_DIAGNOSTIC", "false").strip().lower() == "true"
...
# [إضافة — تشخيص عميق اختياري (DEEP_DIAGNOSTIC)، البند ١ بالتحليل المرفق]
# مسبار تسريب Runtime.enable عبر CDP — بلا أي اتصال يدوي بـCDPSession من
# كودنا، وبلا أي مكتبة خارجية. الآلية: خاصية بـgetter على كائن تُمرَّر
# مباشرة لـconsole.log. بمتصفح مستخدم عادي (بلا أي أداة أتمتة تُفعِّل نطاق
# Runtime عبر CDP) لا يُستدعى الـgetter إطلاقًا — console.log لا يقرأ كل
# خاصية فعليًا إلا عند تسلسل معاينة الكائن (object preview) لإرسالها عبر
# البروتوكول. لكن حين يكون نطاق Runtime مفعّلًا (الافتراضي بمعظم أدوات
# الأتمتة القائمة على CDP، ومنها Playwright بحسب المسار الداخلي)، يُسلسِل
# Chrome هذه المعاينة فورًا — وهذا التسلسل يستدعي كل getter فعليًا. نقيس
# هنا فقط: هل تسرّب الاستدعاء أم لا، بصرف النظر عمّن فعّل Runtime.enable.
CDP_RUNTIME_LEAK_PROBE_JS = """() => new Promise((resolve) => {
    let leaked = false;
    const obj = {};
    Object.defineProperty(obj, 'probe', {
        get() { leaked = true; return 'x'; },
        enumerable: true,
    });
    try { console.log(obj); } catch (e) {}
    setTimeout(() => resolve(leaked), 120);
})"""

# [إضافة — تشخيص عميق اختياري، البند ٢ بالتحليل المرفق] مسبار توقيع
# sourceURL — سكربتات page.evaluate() بأدوات الأتمتة القائمة على CDP تحمل
# غالبًا توقيع sourceURL افتراضيًا مميّزًا (مثال موثّق: نمط "pw:" الخاص
# بـPlaywright) قابل للرصد من *داخل* الصفحة نفسها عبر ستاك استثناء JS
# عادي — أي موقع يفحص Error().stack عند أي خطأ (نمط كشف موثّق حي، لا
# افتراضي نظري) يمكنه تمييز أن التنفيذ يحدث عبر أتمتة لا متصفح مستخدم.
SOURCEURL_LEAK_PROBE_JS = """() => {
    try { throw new Error('__probe__'); } catch (e) { return (e && e.stack) || null; }
}"""

_SOURCEURL_SUSPECT_PATTERNS = (
    "pw:", "playwright", "puppeteer_evaluation_script", "__puppeteer_evaluation_script__",
)


async def _capture_cdp_runtime_leak(page) -> bool | None:
    """[تشخيص عميق] True لو تسرّب استدعاء getter عبر console.log — إشارة
    مباشرة أن نطاق Runtime مفعّل بهذا السياق تحديدًا (بصرف النظر عن
    السبب). None لو تعذّر القياس (لا يُفسَّر كـFalse — الفرق مهم هنا)."""
    try:
        return await page.evaluate(CDP_RUNTIME_LEAK_PROBE_JS)
    except Exception:
        return None


async def _capture_sourceurl_leak(page) -> dict:
    """[تشخيص عميق] يفحص ستاك استثناء JS عادي بحثًا عن توقيع أداة أتمتة
    معروف. الستاك الخام يُرفَق كاملًا بالتقرير (بلا اقتطاع) للتحقق اليدوي
    الفعلي بدل الاكتفاء بقائمة الأنماط الثابتة أعلاه فقط."""
    try:
        stack = await page.evaluate(SOURCEURL_LEAK_PROBE_JS)
    except Exception:
        return {"stack": None, "suspect_patterns_found": [], "error": "تعذّر تنفيذ المسبار"}
    stack_l = (stack or "").lower()
    found = [p for p in _SOURCEURL_SUSPECT_PATTERNS if p in stack_l]
    return {"stack": stack, "suspect_patterns_found": found}


async def _capture_deep_diagnostic_signals(page) -> dict:
    """[تشخيص عميق] يجمع مسابر البروتوكول الجديدة (البندان ١-٢) بدالة
    واحدة — تُستدعى فقط عند DEEP_DIAGNOSTIC=true، مرّة بالسياق بstealth
    ومرّة بالسياق المرجعي بلا stealth، لعزل: هل التسريب مصدره طبقة JS
    (stealth يعالجها أصلًا) أم طبقة بروتوكول CDP (stealth لا يلمسها إطلاقًا
    لأنه تصحيح JS لا تصحيح بروتوكول — راجع تعليق _STEALTH.apply_stealth_async)."""
    return {
        "cdp_runtime_enable_leak": await _capture_cdp_runtime_leak(page),
        "sourceurl_leak": await _capture_sourceurl_leak(page),
    }
...
async def _no_stealth_reference_probe(browser, url: str) -> dict:
    ...
    *بلا* أي تمويه إطلاقًا: لا _STEALTH، ولا حتى فلاج الإطلاق
    ...
    result = {
        ...
        # [إضافة — تشخيص عميق اختياري] None لو DEEP_DIAGNOSTIC معطَّل — لا
        # يُفسَّر غياب القيمة كـ"لا تسريب"، بل كـ"لم يُقَس إطلاقًا هذه المرة".
        "deep_diagnostic": None,
    }
    try:
        async with async_playwright() as p_clean:
            ...
            try:
                ...
                try:
                    ...
                    if DEEP_DIAGNOSTIC:
                        result["deep_diagnostic"] = await _capture_deep_diagnostic_signals(page)
...
async def _browser_probe(browser, url: str, diag_dir: Path, slug: str) -> dict:
    result = {
        ...
        # [إضافة — تشخيص عميق اختياري] None لو DEEP_DIAGNOSTIC معطَّل، بنفس
        # منطق _no_stealth_reference_probe أعلاه — راجع تعليق ذلك الحقل.
        "deep_diagnostic": None,
    ...
    if DEEP_DIAGNOSTIC:
        # [إضافة — تشخيص عميق اختياري] يُقاس هنا بالسياق الرئيسي (بstealth
        # + فلاج --disable-blink-features=AutomationControlled معًا، مطابقةً
        # للإنتاج) — يُقارَن لاحقًا بنظيره بالسياق المرجعي بلا stealth
        # (_no_stealth_reference_probe) ضمن نفس stealth_comparison أدناه.
        result["deep_diagnostic"] = await _capture_deep_diagnostic_signals(page)
    ...
    # [إضافة — تشخيص عميق اختياري، البند ٣ بالتحليل المرفق] مقارنة صريحة
    # بstealth مقابل بلاه على طبقة CDP-protocol فقط — بيانات خام بلا
    # استنتاج مُدمَج، تُستخدَم لاحقًا لبناء detection_surface_map بـ
    # diagnose_url. القيمة الجوهرية هنا: لو التسريب True بكلا السياقين
    # (بstealth وبلاه)، فهذا دليل مباشر أن stealth الحالي لا يعالج هذه
    # الطبقة إطلاقًا (متوقَّع تمامًا — stealth يصحّح JS لا بروتوكول CDP).
    if DEEP_DIAGNOSTIC:
        deep_stealth = result.get("deep_diagnostic") or {}
        deep_no_stealth = no_stealth.get("deep_diagnostic") or {}
        result["stealth_comparison"]["deep_diagnostic_comparison"] = {
            "cdp_runtime_enable_leak_with_stealth": deep_stealth.get("cdp_runtime_enable_leak"),
            "cdp_runtime_enable_leak_without_stealth": deep_no_stealth.get("cdp_runtime_enable_leak"),
            "leak_present_regardless_of_stealth": (
                deep_stealth.get("cdp_runtime_enable_leak") is True
                and deep_no_stealth.get("cdp_runtime_enable_leak") is True
            ),
            "sourceurl_leak_with_stealth": deep_stealth.get("sourceurl_leak"),
            "sourceurl_leak_without_stealth": deep_no_stealth.get("sourceurl_leak"),
        }
...
async def diagnose_url(browser, url: str, diag_dir: Path, runner_info: dict | None = None) -> dict:
    ...
    if DEEP_DIAGNOSTIC:
        print("🧬 تشخيص عميق (بروتوكولي — CDP Runtime.enable + sourceURL) مفعّل لهذه التشغيلة")
    print("═" * 60)
    ...
    if sc:
        ...
        ddc = sc.get("deep_diagnostic_comparison")
        if ddc:
            leak_ws = ddc.get("cdp_runtime_enable_leak_with_stealth")
            leak_ns = ddc.get("cdp_runtime_enable_leak_without_stealth")
            print(f"   🧬 [عميق] تسريب Runtime.enable عبر CDP — بstealth: {leak_ws} | بلا-stealth: {leak_ns}")
            if ddc.get("leak_present_regardless_of_stealth"):
                print("   🚨 [عميق] التسريب حاضر بكلا السياقين — طبقة بروتوكول لا يعالجها stealth الحالي إطلاقًا")
            su_ws = (ddc.get("sourceurl_leak_with_stealth") or {}).get("suspect_patterns_found")
            su_ns = (ddc.get("sourceurl_leak_without_stealth") or {}).get("suspect_patterns_found")
            if su_ws or su_ns:
                print(f"   🧬 [عميق] توقيعات sourceURL مشبوهة بستاك الاستثناء — بstealth: {su_ws} | بلا-stealth: {su_ns}")
    ...
    # [إضافة — البند ٣ بالتحليل المرفق] خريطة موحّدة لكل طبقة كشف مقاسة —
    # بدل نتائج متفرقة بحقول منفصلة، توضح صراحةً *أي طبقة تحديدًا* مسؤولة
    # عن أي فرق مُلاحَظ لهذا الموقع: JS-fingerprint (stealth_comparison
    # القديم) × CDP-protocol (deep_diagnostic الجديد، اختياري) × hotlink ×
    # rate-limit. بيانات خام مجمّعة فقط، بلا استنتاج/قرار مُدمَج بالكود.
    sc_map = browser_r.get("stealth_comparison") or {}
    ddc_map = sc_map.get("deep_diagnostic_comparison") or {}
    detection_surface_map = {
        "js_fingerprint_layer": {
            "webdriver_flag_hidden_by_stealth": sc_map.get("webdriver_flag_hidden_by_stealth"),
            "challenge_outcome_differs_with_stealth": sc_map.get("challenge_outcome_differs"),
            "image_count_differs_with_stealth": sc_map.get("image_count_differs"),
        },
        "cdp_protocol_layer": {
            "tested": DEEP_DIAGNOSTIC,
            "runtime_enable_leak_with_stealth": ddc_map.get("cdp_runtime_enable_leak_with_stealth"),
            "runtime_enable_leak_without_stealth": ddc_map.get("cdp_runtime_enable_leak_without_stealth"),
            "leak_present_regardless_of_stealth": ddc_map.get("leak_present_regardless_of_stealth"),
            "sourceurl_suspect_patterns_with_stealth": (ddc_map.get("sourceurl_leak_with_stealth") or {}).get("suspect_patterns_found"),
            "sourceurl_suspect_patterns_without_stealth": (ddc_map.get("sourceurl_leak_without_stealth") or {}).get("suspect_patterns_found"),
        },
        "hotlink_layer": {
            "referer_only_sufficient": first_hp.get("referer_only_sufficient"),
            "direct_http_success": first_hp.get("direct_http_success"),
        },
        "rate_limit_layer": {
            "rate_limited_detected": rl.get("rate_limited_detected"),
            "static_block_detected": rl.get("static_block_detected"),
    ...
    report = {
        ...
        "detection_surface_map": detection_surface_map,
...
async def run_diagnostic_mode(chapter_urls: list[str]) -> None:
    ...
    print(f"🧬 تشخيص عميق (بروتوكولي، DEEP_DIAGNOSTIC): {'مفعّل — أبطأ (مسبارا CDP + sourceURL إضافيَّان بكل جلسة)' if DEEP_DIAGNOSTIC else 'مُعطَّل (افتراضي — تشخيص عادي فقط)'}")
