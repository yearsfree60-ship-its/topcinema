...
async def attempt_click_turnstile_checkbox(page) -> dict:
    ...
    يُعيد قاموسًا تشخيصيًا خامًا فقط (iframe_found/clicked/error) — لا
    حكمًا نهائيًا على نجاح الحل؛ المستدعي يُعيد classify_challenge_page
    بعد الانتظار وإعادة التحميل ليقرر فعليًا هل انحل التحدي."""
    result = {"iframe_found": False, "clicked": False, "error": None}
    try:
        iframe_el = await page.wait_for_selector(
            TURNSTILE_IFRAME_SELECTOR, timeout=8000, state="attached"
        )
    except Exception as e:
        result["error"] = f"لم يُعثر على إطار Turnstile خلال 8 ثوانٍ: {e}"
        return result
...
async def _browser_probe(browser, url: str, diag_dir: Path, slug: str) -> dict:
    ...
    if result["protection_category"] == "final_block":
        ...
        print("  🛡️ تحدٍّ متصفح قابل للحل — محاولة نقر مربع Turnstile (حل مجاني، غير مضمون)...")
        click_result = await attempt_click_turnstile_checkbox(page)
        result["turnstile_click_attempt"] = click_result
        if click_result["clicked"]:
            print("  🖱️ نُقر مربع Turnstile — انتظار معالجة الخادم للنقرة...")
        elif click_result["iframe_found"]:
            print(f"  ⚠️ إطار Turnstile موجود لكن تعذّر تحديد مربع نقر صالح: {click_result['error']}")
        else:
            print(f"  ℹ️ لا إطار Turnstile تفاعلي ظاهر: {click_result['error']}")
        await page.wait_for_timeout(5000)
        try:
            await page.reload(wait_until="load", timeout=NAV_TIMEOUT_MS)
            ...
            still_category = await classify_challenge_page(page)
            result["protection_category"] = still_category
            result["challenge_detected"] = still_category != "none"
            result["challenge_resolved_after_reload"] = still_category == "none"
            try:
                result["title"] = await page.title()
            except Exception:
                pass
...
async def diagnose_url(browser, url: str, diag_dir: Path, runner_info: dict | None = None) -> dict:
    ...
    tca = browser_r.get("turnstile_click_attempt")
    if tca is not None:
        outcome = "نُقر فعليًا" if tca.get("clicked") else ("إطار موجود، فشل النقر" if tca.get("iframe_found") else "لا إطار تفاعلي")
        solved_note = " ← نجح الحل" if tca.get("clicked") and browser_r["challenge_resolved_after_reload"] else (" ← لم يُحَل رغم النقر" if tca.get("clicked") else "")
        print(f"   🖱️ محاولة نقر Turnstile (حل مجاني): {outcome}{solved_note}")
