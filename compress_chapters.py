...
def _get_paddleocr_engine():
    ...
    if _PADDLEOCR_ENGINE is None:
        ...
        _PADDLEOCR_ENGINE = PaddleOCR(lang="en", use_angle_cls=False, show_log=False)
    return _PADDLEOCR_ENGINE


def ocr_extract_english_sync(raw_bytes: bytes) -> list[dict]:
    """يُشغَّل عبر asyncio.to_thread (PaddleOCR متزامن). يُرجع صناديق نص خام
    بلا أي فرز/تجميع بعد: كل عنصر {bbox: [[x,y]×4], text: str, confidence: float}."""
    import numpy as np
    engine = _get_paddleocr_engine()
    img = Image.open(BytesIO(raw_bytes)).convert("RGB")
    result = engine.ocr(np.array(img), cls=False)
    items: list[dict] = []
