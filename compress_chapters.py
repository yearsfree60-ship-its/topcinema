...
async def looks_like_challenge_page(page) -> bool:
    ...
    try:
        ...
        return False
    if sum(1 for m in CHALLENGE_BODY_MARKERS if m in body_text) >= 2:
        return True
    ...
    # آلية جديدة) يوسّع الكشف الإنتاجي بصفر تكلفة إضافية حقيقية.
...
async def _no_stealth_reference_probe(browser, url: str) -> dict:
    """[إضافة — تشخيص واقع الإنتاج الفعلي] يفتح سياقًا منفصلًا *بلا* أي
    تمويه (لا _STEALTH ولا حتى الترقيع اليدوي القديم) — يمثّل بالضبط ما
    كانت عليه بروفايلات المتصفح بالإنتاج قبل إضافة playwright-stealth.
    ...
    بنتيجة الممر الرئيسي (الذي يستخدم _STEALTH الآن، مطابقةً للإنتاج) في
    _recommend_profile — بدل توصية عامة، سبب مبني على رقمين فعليين."""
    result = {
        "navigated": False, "challenge_detected": False, "images_after_wait": None,
...
async def _browser_probe(browser, url: str, diag_dir: Path, slug: str) -> dict:
    ...
    # ومقارنته رقميًا بالممر الرئيسي أعلاه (الذي يستخدم stealth الآن). يجيب:
    # هل stealth غيّر شيئًا فعليًا لهذا الموقع تحديدًا، أم لا فرق (فالحماية
    # هنا تعتمد على شيء آخر — IP/rate-limit على الأرجح)؟ راجع _recommend_profile
    # لكيفية استخدام هذه المقارنة بالتوصية النهائية.
...
def _recommend_profile(static_r: dict, browser_r: dict, rate_limit_r: dict | None = None, consistency_diffs: list[str] | None = None) -> dict:
    ...
    # [إضافة] استخدام مقارنة stealth/بلا-stealth الفعلية (راجع _browser_probe)
    # بدل افتراض عام أن "stealth يساعد عادةً" — رقمان حقيقيان لهذا الموقع.
    sc = browser_r.get("stealth_comparison") or {}
    if sc:
        if sc.get("webdriver_flag_hidden_by_stealth") is False:
            reasons.append(
                "⚠️ فحص فعلي: navigator.webdriver بقي ظاهرًا حتى بعد تطبيق playwright-stealth — "
                "الترقيع لم ينجح لهذا الموقع تحديدًا (سبب تقني بالموقع نفسه أو تعارض إصدار)، "
                "يستحق فحصًا يدويًا للقطة الشاشة"
            )
        if sc.get("challenge_outcome_differs") or sc.get("image_count_differs"):
            reasons.append(
                "🎯 مقارنة فعلية: نتيجة التحقق/عدد الصور اختلفت فعليًا بين تشغيلة بلا stealth "
                "وتشغيلة بـstealth — stealth مؤثر فعليًا لهذا الموقع (ليس افتراضًا نظريًا)، "
                "وإبقاؤه مفعّلًا بالإنتاج (كما هو الآن) قرار مبرَّر بدليل رقمي"
            )
        else:
            reasons.append(
                "ℹ️ مقارنة فعلية: لا فرق ملحوظ بين بلا stealth وبstealth (نفس نتيجة التحقق وعدد "
                "الصور تقريبًا) — الحماية هنا على الأرجح تعتمد على شيء غير بصمة المتصفح "
                "(IP/معدل الطلبات مثلًا)، راجع فحص تحديد المعدل أدناه"
            )
    ...
    # [إضافة — المرحلة ج] وقت انتظار مقترَح مبني على منحنى الاستقرار الفعلي
    # المقاس بالمرحلة أ (wait_for_images_elapsed_sec = الزمن حتى استقر عدد
    # الصور فعليًا، أو المهلة القصوى كاملة لو لم يستقر) — بدل CONTENT_WAIT_MS
    # الثابت (20 ثانية افتراضيًا) المطبَّق على كل المواقع بلا تمييز.
    suggested_content_wait_ms = CONTENT_WAIT_MS
    wait_elapsed = browser_r.get("wait_for_images_elapsed_sec")
    if wait_elapsed is not None:
        if wait_elapsed < (CONTENT_WAIT_MS / 1000) * 0.9:
            # استقرت الصور قبل نهاية المهلة الحالية بوضوح — هامش أمان 50%
            # فوق الزمن المقاس فعليًا، بحد أدنى 5 ثوانٍ وبلا تجاوز المهلة
            # الحالية (لا نقترح رقمًا أكبر مما هو مضبوط الآن).
            candidate = max(5000, int(wait_elapsed * 1000 * 1.5))
            suggested_content_wait_ms = min(CONTENT_WAIT_MS, round(candidate / 1000) * 1000)
            if suggested_content_wait_ms != CONTENT_WAIT_MS:
                reasons.append(
                    f"⏱️ الصور استقرت فعليًا (مقاس، لا تخمين) خلال {wait_elapsed}ث — وقت انتظار "
                    f"مقترَح لهذا الموقع {suggested_content_wait_ms}ms بدل {CONTENT_WAIT_MS}ms الثابت لكل المواقع"
                )
        else:
            reasons.append(
                f"⏱️ انتظار استقرار الصور استغرق كامل المهلة الحالية ({CONTENT_WAIT_MS}ms) دون استقرار واضح — "
                "يُبقى وقت الانتظار كما هو بدل تقليصه (تقليصه هنا قد يقطع تحميل صور لم تصل بعد)"
            )

    # [إضافة — المرحلة ج] do_stealth صريح: يُعطَّل فقط لو المقارنة الفعلية
    # (stealth_comparison بـ_browser_probe) أظهرت رقميًا لا فرق حقيقي —
    # لا افتراض عام. الأسباب النصية لهذه المقارنة مُضافة أعلاه بالفعل.
    do_stealth = True
    if sc and not sc.get("challenge_outcome_differs") and not sc.get("image_count_differs"):
        do_stealth = False
    ...
    return {
        ...
        "suggested_content_wait_ms": suggested_content_wait_ms, "do_stealth": do_stealth,
        "suggested_selectors": browser_r.get("suggested_selectors") or [],
    }
...
async def diagnose_url(browser, url: str, diag_dir: Path, runner_info: dict | None = None) -> dict:
    ...
    if sc:
        ...
        print(f"   🕵️ مقارنة stealth: بلا-stealth (تحقق={ns.get('challenge_detected')}, "
              ...
              + (" — فرق فعلي ✅" if (sc.get("challenge_outcome_differs") or sc.get("image_count_differs")) else " — لا فرق ملحوظ"))
        if sc.get("webdriver_flag_hidden_by_stealth") is False:
            print("   ⚠️ navigator.webdriver لم يُخفَ فعليًا حتى بعد stealth لهذا الموقع")
    aw = browser_r.get("adblock_wall") or {}
    if aw.get("detected"):
    ...
    if recommendation["fetch_mode"] == "http":
        ...
        # [إضافة — المرحلة ج] حقول تخصيص إضافية اختيارية — تُدرَج فقط لو
        # اختلفت عن الافتراضي العام (بدل إثقال كل بروفايل بحقول لا فرق لها).
        extra_fields = ""
        if recommendation.get("suggested_content_wait_ms") and recommendation["suggested_content_wait_ms"] != CONTENT_WAIT_MS:
            extra_fields += f', "content_wait_ms": {recommendation["suggested_content_wait_ms"]}'
        if not recommendation.get("do_stealth", True):
            extra_fields += ', "do_stealth": False'
        if recommendation.get("suggested_selectors"):
            extra_fields += f', "extra_selectors": {recommendation["suggested_selectors"]!r}'
        print(
            f'   "اسم_الموقع": {{"label": "...", "fetch_mode": "browser", '
            f'"do_scroll": {recommendation["do_scroll"]}, "do_widget_filter": {recommendation["do_widget_filter"]}{extra_fields}}},'
        )
